...
/// the image's stable position: top-level images retain page-object order and
/// nested Form images follow in depth-first order. A later embed pass uses it
/// to pull pixel bytes with `render_image_object`.
#[doc(hidden)]
#[derive(Debug, Clone)]
pub struct ImageRef {
    pub id: String,
